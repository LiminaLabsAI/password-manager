...
| Supervisor (per repo, fresh opencode session pinned to repo cwd) | `<repo>/specs/phases/phase-N-<slug>/*` + `<repo>/.momentum/runs/dispatch-run-<id>.json` | Its phase brief + contract + history.md tail |
...
- The user wants ONE opencode session driving the whole feature, not three serial sessions.
...
- **Supervisor declaration**: `.opencode/agents/swarm-supervisor.md` (opencode agent, `mode: subagent`). Each supervisor reads `core/swarm/supervise.md` end-to-end on boot.
- **cwd pinning**: `opencode run --dir <repoPath>` pins each supervisor's working directory at process boundary — native, no MCP shim required (the supervise.md cwd invariant holds by construction).
- **Spawn dispatch**: the CLI floor (`momentum swarm start --spawn …`) dispatches each spawn directive through `adapters/opencode/adapter.js::spawn()`. The adapter shells `opencode run --dir <repoPath> --agent swarm-supervisor` with the boot directive as the message.
- **Hook integration**: the momentum plugin (`.opencode/plugins/momentum.js`) applies uniformly inside each supervisor. The brainstorm gate keeps supervisors from drifting into spec edits during phase implementation.
...
**Step 2 — Spawn Wave 1 supervisors via the opencode adapter.**
...
`bin/swarm.js` dispatches each directive through `adapters/opencode/adapter.js::spawn()`, which shells `opencode run --dir <repoPath> --agent swarm-supervisor`. If `opencode` is not on PATH, the CLI surfaces the spawn directives and exits non-zero — degrade by reporting the directives to the user (they can launch sessions manually) and continue with conductor polling on the existing manifest.
...
Issue a single-use focus token for `<repo>` and hand control to a second opencode session. Use when one repo needs sustained one-on-one attention without halting the rest of the swarm.
...
5. Audit-logs `focus`.
6. Prints a spawn directive — run a second `opencode --dir <eco>` session in a separate terminal, then inside that session call `momentum swarm join <swarm-id> --token <token>`.
...
Attach the current opencode session to `<swarm-id>` as a co-conductor. Adds the session to `sessions[]` (idempotent — touch on re-join), auto-renews any repos the session already owns, and optionally consumes a transfer token or claims a specific repo.
...
- `opencode` CLI not on PATH → degrade to dry-run + manual spawn instructions.
- (No MCP cwd shim needed on opencode — `--dir` pins each supervisor's cwd at process boundary.)
